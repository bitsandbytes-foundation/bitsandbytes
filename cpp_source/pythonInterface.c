...
{ optimizer32bit<gtype, oname>(g, p, state1, state2, unorm, max_unorm, beta1, beta2, eps, weight_decay, step, lr, is_sparse, gnorm_scale, n); } \
...
{  \
	optimizerStatic8bit<gtype, oname>(g, p, state1, state2, beta1, beta2, eps, step, lr, \
...
extern "C"
{
  ...
  void cdequantize_blockwise_fp32(float *code, unsigned char *A, float *absmax, float *out, const int n){ dequantizeBlockwise_fp32(code, A, absmax, out, n); }
	...
	{ name##32bit_g##gbits(g, p, state1, state2, unorm, max_unorm, beta1, beta2, eps, weight_decay, step, lr, is_sparse, gnorm_scale, n); } \
	...
	void c##name##_static_8bit_g##gbits(gtype* p, gtype* g, unsigned char* state1, unsigned char* state2, \
                ...
                float weight_decay, float gnorm_scale, int n) \
  {  \
	    name##_static_8bit_g##gbits(g, p, state1, state2, beta1, beta2, eps, step, lr, \
			                                 quantiles1, quantiles2, max1, max2, new_max1, new_max2, weight_decay, gnorm_scale, n); \
  } \
