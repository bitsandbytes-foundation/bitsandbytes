...
def test_matmul_4bit(
    ...
    # transpose_B=True: B is [N, K] -- standard convention
    # transpose_B=False: B is [K, N] -- deprecated orientation, emits DeprecationWarning
    dimA = (dim2, dim3)
    dimB = (dim4, dim3) if transpose_B else (dim3, dim4)
    ...
    for i in range(3):
        ...
        if transpose_B:
            out_torch = torch.matmul(A, B.t())
            out_bnb = bnb.matmul_4bit(A, B2, quant_state, bias=bias2)
        else:
