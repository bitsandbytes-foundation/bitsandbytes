...
if __name__ == '__main__':
    ...
    speed_sglint8 = [1898, 680, 369.5]
    speed_llmint8 = [858, 309, 170]


    speed_sglint8 = [1./x for x in speed_sglint8]
    speed_llmint8 = [1./x for x in speed_llmint8]


    sppedup_over_llmint8 = [-100 * (speed_sglint8[i] - speed_llmint8[i])/speed_llmint8[i] for i in range(len(speed_llmint8))]
    ...
    # ax.plot([0,1, 2], speedup_over_my_standard, color=cmap(0.5), linestyle='--', label='speedup over pytorch optimized linear', marker='s', markersize=8)
    ax.plot([0,1, 2], sppedup_over_llmint8, color=cmap(0.5), linestyle=':', label='speedup over LLM.int8()', marker='^', markersize=8)
    ...
    plt.savefig('tests/triton_tests/plot5.pdf', bbox_inches='tight')
