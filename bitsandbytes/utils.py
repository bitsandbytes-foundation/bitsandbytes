...
def outlier_hook(module, input):
    ...
    if hvalue not in tracer.hvalue2outlier_idx:
        ...
        if len(tracer.outliers) > 1:
            ...
            # (2) magnitude > 6 test
            dims = (torch.abs(input[0])
                    > 6).sum(dim=list(range(len(input[0].shape) - 1)))
...
def find_outlier_dims(weight, reduction_dim=0, zscore=4.0, topk=None, rdm=False):
    if rdm:
        return torch.randint(
            0, weight.shape[1], size=(topk, ), device=weight.device).long()
...
def replace_linear(
        ...
        post_processing_function: Optional[str] = None) -> Module:
    """
    ...
    for name, module in model.named_children():
        if any(isinstance(child, Module) for child in module.children()):
            replace_linear(
                module, linear_replacement, skip_modules, copy_weights,
