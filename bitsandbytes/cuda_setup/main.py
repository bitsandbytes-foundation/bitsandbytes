...
class CUDASetup:
    ...
    def generate_instructions(self):
        ...
        elif self.cuda_version_string[:2] == '11' and int(
                self.cuda_version_string[2]) > 0:
    ...
    def manual_override(self):
        if (torch.cuda.is_available() and 'BNB_CUDA_VERSION' in os.environ
                and len(os.environ['BNB_CUDA_VERSION']) > 0):
            warn(
                f'\n\n{"="*80}\n'
...
def warn_in_case_of_duplicates(results_paths: Set[Path]) -> None:
    if len(results_paths) > 1:
        results_paths_str = ', '.join(f'`{f!s}`' for f in results_paths)
        warning_msg = ' '.join([
            f"Found duplicate {CUDA_RUNTIME_LIBS} files: {results_paths_str}).. ",
...
def determine_cuda_runtime_lib_path() -> Union[Path, None]:
    ...
    cuda_runtime_libs = set()
    for env_var_value in remaining_candidate_env_vars.values():
        cuda_runtime_libs.update(find_cuda_lib_in(env_var_value))
    ...
    cuda_setup.add_log_entry(
        f'DEBUG: Possible options found for libcudart.so: {cuda_runtime_libs}')
...
def evaluate_cuda_setup():
    ...
    if 'BITSANDBYTES_NOWELCOME' not in os.environ or str(
            os.environ['BITSANDBYTES_NOWELCOME']) == '0':
        cuda_setup.add_log_entry('')
