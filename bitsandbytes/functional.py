...
def dequantize_blockwise(
    ...
    if quant_state is None:
        quant_state = QuantState(
            absmax=absmax, code=code, blocksize=blocksize, dtype=torch.float32
        )
...
def quantize_fp4(
    ...
    return quantize_4bit(
        A, absmax, out, blocksize, compress_statistics, "fp4", quant_storage
    )
...
def quantize_nf4(
    ...
    return quantize_4bit(
        A, absmax, out, blocksize, compress_statistics, "nf4", quant_storage
    )
...
def quantize_no_absmax(
    A: Tensor, code: Tensor, out: Optional[torch.Tensor] = None
) -> Tensor:
    """
...
def optimizer_update_8bit(
    ...
    prev_device = pre_call(g.device)
    is_on_gpu(
        [g, p, state1, state2, unorm_vec, qmap1, qmap2, max1, max2, new_max1, new_max2]
    )
...
def gemv_4bit(
    ...
    if out is None:
        if len(A.shape) == 3:
            out = torch.empty(
                size=(A.shape[0], A.shape[1], bout), dtype=A.dtype, device=A.device
            )
        else:
...
def igemmlt(A, B, SA, SB, out=None, Sout=None, dtype=torch.int32):
    ...
    if shapeA[0] == 0 and dimsA == 2:
        ...
        return torch.empty(
            tuple(shapeA[:2] + [shapeB[0]]), device=A.device, dtype=torch.float16
        )
...
def pipeline_test(A, batch_size):
    ...
    lib.cpipeline_test(
        get_ptr(A), get_ptr(out), ct.c_size_t(A.numel()), ct.c_size_t(batch_size)
    )
    return out
