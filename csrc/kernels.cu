          ...
          for(int k = 0; k < num_items; k++)
          {
              ...
              // we do texture cache reads (__ldg) on dequant_stats which should be super fast
            {
              ...
              if(valB != 0.0 && valA != 0.0)
                local_valC[j+k] += ((float)smem_dequant_stats[idx+k-local_idx_col_B_offset])*C*valB*valA;
            }
            else
...
template __global__ void kTransformFormatToRow<256, 8, 32, 32*8, 0, COL_TURING>(char *__restrict__ const A, char *out, int rows, int cols, int tiledCols, int outRows, int outCols);
template __global__ void kTransformFormatToRow<256, 8, 32, 32*8, 0, COL_AMPERE>(char *__restrict__ const A, char *out, int rows, int cols, int tiledCols, int outRows, int outCols);
