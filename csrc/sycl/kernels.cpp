                        ...
                        const sycl::nd_item<3> &item_ct1)
{
  ...
  */
  int smem_qidx[BLOCK_ESTIMATE];
  sycl::buffer<T, 1> buff_A(A,sycl::range<1>(NUM_ESTIMATE));
  sycl::buffer<T>
  for (unsigned int i = base_idx; i < n_full; i += item_ct1.get_group_range(2)*BLOCK_ESTIMATE)
  {
      ...
      */
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          size_t temp_storage_size = group_load::get_local_memory_size(NUM_ESTIMATE);
          sycl::local_accessor<uint8_t, 1> tacc(
            temp_storage_size, h);
      ...
      */
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          size_t temp_storage_size = group_radix_sort::get_local_memory_size(NUM_ESTIMATE);
          sycl::local_accessor<int, 1> tacc(
            temp_storage_size, h);
          ...
          h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 256), sycl::range<3>(1, 1, 256)),
            [=](sycl::nd_item<3> item) {
              auto *tmp = tacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              group_radix_sort(tmp).sort_blocked_to_striped(item, vals);
            });
                ...
                const sycl::nd_item<3> &item_ct1)
{
  ...
  sycl::buffer<float, 1> buff_state1(state1,sycl::range<1>(NUM_VALS));
  sycl::buffer<float, 1> buff_state2(state2,sycl::range<1>(NUM_VALS));
  ...
  for (unsigned int i = base_idx; i < n_full; i += item_ct1.get_group_range(2)*BLOCK_SIZE)
  {
      ...
      */
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          size_t temp_storage_size = group_load::get_local_memory_size(NUM_VALS);
          sycl::local_accessor<uint8_t, 1> tacc(
            temp_storage_size, h);
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 256), sycl::range<3>(1, 1, 256)),
            [=](sycl::nd_item<3> item) {
              auto *d = dacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              auto *tmp = tacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              group_load(tmp).load(item,item.get_local_linear_id(), d, s1_vals);
            });
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 256), sycl::range<3>(1, 1, 256)),
            [=](sycl::nd_item<3> item) {
              auto *d = dacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              auto *tmp = tacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              group_load(tmp).load(item,item.get_local_linear_id(), d, s2_vals);
            });
                ...
                const sycl::nd_item<3> &item_ct1)
{
  ...
  sycl::buffer<T, 1> buff_g(g, sycl::range<1>(NUM_PER_THREAD));
  sycl::buffer<T, 1> buff_p(p, sycl::range<1>(NUM_PER_THREAD));
  sycl::buffer<float, 1> buff_state1(state1,sycl::range<1>(NUM_PER_THREAD));
  sycl::buffer<float, 1> buff_state2(state2,sycl::range<1>(NUM_PER_THREAD));
  ...
  for (unsigned int i = base_idx; i < n_full; i += item_ct1.get_group_range(2)*TH*NUM_PER_THREAD)
  {
      ...
      */
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          size_t temp_storage_size = group_load::get_local_memory_size(NUM_PER_THREAD);
          sycl::local_accessor<uint8_t, 1> tacc(
            temp_storage_size, h);
          sycl::accessor dacc(buff_g[i], h, sycl::read_write);
      ...
      */
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          size_t temp_storage_size = group_store::get_local_memory_size(NUM_PER_THREAD);
          sycl::local_accessor<uint8_t, 1> tacc(
            temp_storage_size, h);
          sycl::accessor dacc(buff_p[i], h, sycl::read_write);
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 256), sycl::range<3>(1, 1, 256)),
            [=](sycl::nd_item<3> item) {
              auto *d = dacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              auto *tmp = tacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              group_store(tmp).store(item,item.get_local_linear_id(), d, s1_vals);
            });
      ...
      dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 256), sycl::range<3>(1, 1, 256)),
            [=](sycl::nd_item<3> item) {
              auto *d = dacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              auto *tmp = tacc.get_multi_ptr<sycl::access::decorated::yes>().get();
              group_store(tmp).store(item,item.get_local_linear_id(), d, s2_vals);
            });
                ...
                float *smem_exchange1, float *smem_exchange2)
{
    ...
    sycl::buffer<T, 1> buff_g(g, sycl::range<1>(N_PER_TH));
    sycl::buffer<T, 1> buff_p(p, sycl::range<1>(N_PER_TH));
    sycl::buffer<unsigned char, 1> buff_state1(state1,sycl::range<1>(N_PER_TH));
    sycl::buffer<unsigned char, 1> buff_state2(state2,sycl::range<1>(N_PER_TH));
    ...
    for (unsigned int i = base_idx; i < n_full; i += item_ct1.get_group_range(2)*BLOCK_SIZE)
    {
        ...
        */
        ...
        dpct::get_in_order_queue().submit([&](sycl::handler &h) {
          ...
          size_t temp_storage_size = group_load::get_local_memory_size(N_PER_TH);
          sycl::local_accessor<uint8_t, 1> tacc(
            temp_storage_size, h);
          sycl::accessor dacc(buff_g[i], h, sycl::read_write);
...
template<typename T, int THREADS, int ITEMS_PER_THREAD, int TILE_ROWS, int TILE_COLS, int SPARSE_DECOMP> void kgetColRowStats(T * __restrict__ A, float *rowStats, float *colStats, int * nnz_count_row, float nnz_threshold, int rows, int cols, int tiledRows, int tiledCols,  const sycl::nd_item<3> &item_ct1, float *smem_row_absmax_values,  int *smem_row_nnz_values)
{
  ...
  */
  sycl::buffer<T, 1> buff_A(A, sycl::range<1>(ITEMS_PER_THREAD));
...
template void kPreconditionOptimizer32bit1State<gtype, oname, 4096, 8>(gtype* g, gtype* p, \
                float* state1, float *unorm, \
                const float beta1, const float beta2, const float eps, const float weight_decay, \
                const int step, const float lr, const float gnorm_scale, const int n, const sycl::nd_item<3> &item_ct1; \
...
template void kOptimizer32bit1State<gtype, oname>(gtype* g, gtype* p, float* state1, float *unorm, const float max_unorm, const float param_norm, \
    const float beta1, const float beta2, const float eps, const float weight_decay,const int step, const float lr, const float gnorm_scale, const bool skip_zeros, const int n, const sycl::nd_item<3> &item_ct1); \
...
template void kPreconditionOptimizerStatic8bit1State<gtype, oname>(gtype* p, gtype* __restrict__ const g, unsigned char*__restrict__  const state1,  \
                ...
                const int n, const sycl::nd_item<3> &item_ct1, float *smem_quantiles1); \
...
template void kPreconditionOptimizerStatic8bit2State<gtype, oname>(gtype* p, gtype* __restrict__ const g, unsigned char*__restrict__  const state1, unsigned char* __restrict__ const state2, \
                ...
                float* max1, float* max2, float* new_max1, float* new_max2, \
                const float gnorm_scale,  \
                const int n, const sycl::nd_item<3> &item_ct1,  float *smem_quantiles1, float *smem_quantiles2); \
...
template void kOptimizerStatic8bit2StateBlockwise<gtype, oname, block_size, num_per_thread>(gtype* p, gtype* __restrict__ const g, unsigned char* state1, unsigned char* state2, \
                ...
                float* absmax1, float* absmax2,  \
                float weight_decay, \
                const float gnorm_scale, const bool skip_zeros, const int n, const sycl::nd_item<3> &item_ct1, sycl::local_accessor<float, 2> smem_quantiles1, sycl::local_accessor<float, 2> smem_quantiles2, float *smem_exchange1, float *smem_exchange2); \
...
template void kOptimizerStatic8bit1StateBlockwise<gtype, oname, block_size, num_per_thread>( \
		gtype* p, gtype* __restrict__ const g, unsigned char* state1, \
                ...
                float* absmax1, \
                float weight_decay, \
                const float gnorm_scale, const bool skip_zeros, const int n, const sycl::nd_item<3> &item_ct1, sycl::local_accessor<float, 2> smem_quantiles1, float *smem_exchange1); \
