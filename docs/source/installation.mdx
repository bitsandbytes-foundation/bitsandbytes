...
> [!TIP]
> Intel CPU backend only supports build from source for now, please following the instruction below.

Same as cuda, you can compile bitsandbytes from source for Linux and Windows systems. Installing from source allows for more build options with different CMake configurations.
...
We recommended you to install **GCC >= 11** and required to have at least **GCC >= 6**.
