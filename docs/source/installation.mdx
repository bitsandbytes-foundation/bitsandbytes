...
> [!TIP]
> If `rocminfo` is unavailable or fails in your runtime environment, install it if possible. Otherwise set `BNB_ROCM_GPU_ARCH` (for example `BNB_ROCM_GPU_ARCH=gfx90a`) as a fallback.
