...
all: $(ROOT_DIR)/dependencies/cub $(BUILD_DIR) $(CUTLASS)
	echo $(CONDA_PREFIX)
	$(NVCC) $(COMPUTE_CAPABILITY) -std=c++14 -Xcompiler '-fPIC' --use_fast_math -Xptxas=-v -dc $(FILES_CUDA) $(INCLUDE) $(LIB) --output-directory $(BUILD_DIR)
	$(NVCC) $(COMPUTE_CAPABILITY) -std=c++14 -Xcompiler '-fPIC' -dlink $(BUILD_DIR)/ops.o $(BUILD_DIR)/kernels.o -o $(BUILD_DIR)/link.o 
